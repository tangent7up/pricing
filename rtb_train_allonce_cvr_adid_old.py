...
df_win_2 = df3.select(df3.ad.paytype.alias("ad_paytype"),*win_col).cache()
try:
    os.mkdir(path + 'old_models')
except:
    pass
try:
    os.mkdir(path + 'old_models/ads/')
except:
    pass
...
for session_path in session_paths:
    ...
    tmp = spark.sql("select ad.adid,count(1) as num from df where has_active = 1 group by ad.adid order by num desc")
    has_click_dict = dict([(i['adid'],i['num']) for i in tmp.rdd.collect()])
    tmp = spark.sql("select ad.adid,count(1) as num from df group by ad.adid order by num desc")
    session_dict = dict([(i['adid'],i['num']) for i in tmp.rdd.collect()])

    if auto_search_ad:
        adid_to_train = [tup[0] for tup in has_click_dict.iteritems() if tup[1]>50]


    df2 = df2.where(df2.ad.adid.isin(adid_to_train))
    df2 = df2.withColumn('click_active', when((df2.has_click == 0) & (df2.has_active == 1), 0).otherwise(df2.has_active))
    df2.createOrReplaceTempView("df")
    tmp = spark.sql("select ad.adid,count(1) as num from df where click_active = 1 group by ad.adid order by num desc")
    click_active_dict = dict([(i['adid'],i['num']) for i in tmp.rdd.collect()])

    df_win_1 = df2.select(df2.ad.paytype.alias("ad_paytype"),*win_col)
    df_win_1 = df_win_1.filter(df_win_1.has_active != 1)
    ...
    adid_to_train = [i for i in adid_to_train if (float(session_dict[i])-float(session_dict_old.get(i,0)))/float(session_dict[i])>0.1 and session_dict_old.get(i,0) < 360000]

    for i in adid_to_train:
        session_dict_old[i] = session_dict[i]
        has_click_dict_old[i] = has_click_dict[i]
        last_train_time[i] = time.time()

    for adid in adid_to_train:
        print 'adid: ',adid
        df_win_ = df_win.filter(df_win.adid == adid).cache()
        count = df_win_.count()
        if count > 500000:
            df_win_ = df_win_.sampleBy("has_active", fractions={0: 400000./count, 1: 1.0}, seed=0)
            sample_percentage = 400000./count
        else:
            sample_percentage = 1.
        ...
        discount = click_active_dict[adid] / np.sum(y_pred_train_cvr) / sample_percentage

        if renew_bid_constand:
            bid_constant = bid_average / click_active_dict[adid] * X_win.shape[0]
        print 'time: ', time.time()-time_a, '\n'
        ...
        try:
            os.mkdir(path + 'old_models/ads/'+str(adid))
        except:
            pass
        with open(path + 'old_models/ads/'+str(adid)+'/model_information','w') as file:
            pickle.dump(model_information,file)
        with open(path + 'old_models/ads/'+str(adid)+'/columns_information','w') as file:
            pickle.dump(columns_information,file)

spark.createDataFrame(session_dict_old.items()).coalesce(1).write.mode("overwrite").format("parquet").save(path +'old_models/session_dict')
spark.createDataFrame(has_click_dict_old.items()).coalesce(1).write.mode("overwrite").format("parquet").save(path +'old_models/has_click_dict')
spark.createDataFrame(last_train_time.items()).coalesce(1).write.mode("overwrite").format("parquet").save(path +'old_models/last_train_time')

with open(path +'old_models/auc_dict','w') as file:
